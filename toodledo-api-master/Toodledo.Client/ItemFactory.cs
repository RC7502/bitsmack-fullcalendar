    ...
    public class ItemFactory<ItemType> : IItemFactory where ItemType : new() 
    {
        ...
        public ItemType Create(XElement element)
        {
            ...
            foreach(var mapping in _mappings)
            {
                ...
                {
                    //special case for due and start
                    if (mapping.Name.ToString() == "duedate" || mapping.Name.ToString() == "startdate")
                    {
                        if(value != "")
                            tempDate = DateTime.Parse(value);

                        mapping.Property.SetValue(item, cast(value, mapping.Type), null);
                    }
                    if (mapping.Name.ToString() == "duetime" || mapping.Name.ToString() == "starttime")
                    {
                        if (value != "")
                        {
                            var time = DateTime.Parse(value);
                            var newDateTime = new DateTime(tempDate.Year, tempDate.Month, tempDate.Day, time.Hour,
                                time.Minute, time.Second);
                            mapping.Property.SetValue(item, newDateTime, null);
                        }
                    }
                    else
                    {
